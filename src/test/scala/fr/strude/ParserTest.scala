...
class ParserTest extends FunSuite with ShouldMatchers {
  ...
  test("extractQuotivalMetadata - should return PRODUCT table name") {
    val result = extractQuotivaldatas(updateFile)(0)
  ...
  test("extractQuotivalMetadata - should return tuple of products, fields and id") {
    extractQuotivaldatas(updateFile)(0) should be (QuotivalData("package", Map("offMarketDate" -> "2013-05-29 00:00:00", "marketStatus" -> "4"), 172211))
  }

  test("extractQuotivalMetadata - should return a List of 2 tuples containing each line's data") {
    extractQuotivaldatas(updateFile).take(2) should be (List(QuotivalData("package", Map("offMarketDate" -> "2013-05-29 00:00:00", "marketStatus" -> "4"), 172211), QuotivalData("package", Map("offMarketDate" -> "2013-05-29 00:00:00", "marketStatus" -> "4"), 172212)))
  }
